...
model_3d::model_3d(QWidget *parent)
: QWidget(parent)
{
    //     7------------------4
    //    / |                / |
...
void model_3d::paintEvent(QPaintEvent *event){
    Q_UNUSED(event);
    ...
    for(int i=0;i<elementArr.count();i++)
    {
      ...
      //单个面的路径
      QPainterPath *element_path = new QPainterPath();
...
QPointF model_3d::getPoint(const QVector3D &vt,int w) const
{
...
void model_3d::changeRotateByRPY(float roll,float pitch, float yaw)
{
